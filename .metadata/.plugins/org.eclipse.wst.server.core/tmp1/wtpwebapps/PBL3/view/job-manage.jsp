...
<html lang="en">
  <head>
    ...
    <title>TheJobs - Quản lý bài đăng</title>
  ...
  <body class="nav-on-header smart-nav bg-alt">
	        ...
	        <c:if test="${role eq 'recruiter'}">
		        <li>
		            ...
		            <ul>
		              ...
		              <li><a href="/PBL3/job-manage" class="active">Danh sách bài đăng</a></li>
		            </ul>
		        </li>
	        </c:if>
    ...
    <header class="page-header bg-img size-lg" style="background-image: url(${pageContext.request.contextPath}/view/assets/img/vaphongweb.jpg)">
      <div class="container no-shadow">
        <h1 class="text-center">Quản lý bài đăng</h1>
        <p class="lead text-center">Đây là danh sách các bài đăng bạn đã nộp. Bạn có thể chỉnh sửa hoặc xóa chúng hoặc thêm một cái mới.</p>
      </div>
    </header>
    <!-- END Page header -->
        ...
        <div class="container">
          <div class="row">
            ...
            <div class="col-xs-12 text-right">
              <br>
              <a class="btn btn-primary btn-sm" href="/PBL3/forward-job-add">Thêm bài tuyển dụng</a>
            </div>
	              ...
	              <div class="item-block">
	                <header>
	                  ...
	                  <div class="hgroup">
	                    ...
	                    <h5><a href="company-detail">${i.name_cpn}</a></h5>
	                  </div>
	                  <div class="header-meta">
	                    <span class="location">${i.address}</span>
	                    <c:if test="${i.status eq 'Chưa duyệt'}"><span class="text-danger">Chưa duyệt</span></c:if>
	                    <c:if test="${i.status eq 'Đã duyệt'}"><span class="text-success">Đã duyệt</span></c:if>
	                    
	                  </div>
	                </header>
	
	                <footer>
	                  <p class="status"><strong>Đăng lúc: </strong><fmt:formatDate pattern="dd-MM-yyyy HH:mm" value="${i.time_post}" />&nbsp;&nbsp;<a class="btn btn-xs btn-info" href="/PBL3/list-job-apply?id_job=${i.id}"> ${i.num_job_apply} Applied</a></p>
					
	                  <div class="action-btn">
	                    <a class="btn btn-xs btn-gray" href="/PBL3/job-update?id_job=${i.id}">Chỉnh sửa</a>
	                    <a class="btn btn-xs btn-danger" href="/PBL3/job-delete?id_job=${i.id}">Xóa</a>
	                  </div>
	                </footer>
	              </div>
	            </div>
